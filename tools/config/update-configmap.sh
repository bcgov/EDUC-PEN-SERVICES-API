...
echo
echo Writing scope DELETE_STUDENT_MERGE
curl -sX POST "https://$SOAM_KC/auth/admin/realms/$SOAM_KC_REALM_ID/client-scopes" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TKN" \
  -d "{\"description\": \"Scope to Delete individual merge records by primary key\",\"id\": \"DELETE_STUDENT_MERGE\",\"name\": \"DELETE_STUDENT_MERGE\",\"protocol\": \"openid-connect\",\"attributes\" : {\"include.in.token.scope\" : \"true\",\"display.on.consent.screen\" : \"false\"}}"
