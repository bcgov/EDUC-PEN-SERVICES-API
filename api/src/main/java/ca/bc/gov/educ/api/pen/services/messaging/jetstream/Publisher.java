...
public class Publisher {
  ...
  private void createOrUpdateStudentEventStream(final Connection natsConnection) throws IOException, JetStreamApiException {
    val streamConfiguration = StreamConfiguration.builder().name(ApplicationProperties.STREAM_NAME).replicas(3).maxMessages(1000000).addSubjects(PEN_SERVICES_EVENTS_TOPIC.toString()).build();
    try {
  ...
  public void dispatchChoreographyEvent(final ServicesEvent event) {
    if (event != null && event.getEventId() != null) {
      ...
      try {
        ...
        val pub = this.jetStream.publishAsync(PEN_SERVICES_EVENTS_TOPIC.toString(), JsonUtil.getJsonBytesFromObject(choreographedEvent));
